...
import {settlementApi, getCartListApi, getAddressListApi, addAddressApi, updateAddressApi, deleteAddressApi} from "@/apis/consumer";
...
const getAddress=async ()=>{
  const consumerId = useConsumerStore().consumerInfo.consumerId
  const res=await getAddressListApi({consumerId})
...
const addAddress=async ()=>{
  ...
  await addAddressApi({
    consumerId: useConsumerStore().consumerInfo.consumerId,
...
const createOrder=async ()=>{
  ...
  const res = await settlementApi({
    consumerId: useConsumerStore().consumerInfo.consumerId,
    orders: carts,
    addressId: curAddress.value.id,
    sum: productPrice.value,
  });
  //更新购物车
...
<template>
          ...
          <div class="address">
            <div class="text">
              ...
              <ul v-else>
                ...
                <li><span>联系方式：</span>{{ curAddress.email }}</li>
  ...
  <el-dialog v-model="showDialog" title="切换收货地址" width="30%" center>
    <div class="addressWrapper">
      <div @click="switchAddress(item)" :class="{active:activeAddress?activeAddress.addressId===item.addressId:curAddress.addressId===item.addressId}" class="text item" v-for="item in checkInfo.addresses"  :key="item.id">
        <ul>
          <li><span>收<i />货<i />人：</span>{{ item.name }} </li>
          <li><span>联系方式：</span>{{ item.email }}</li>
...
.text {
  ...
  &.item {
    ...
    &:hover {
      border-color: $theme-color-4;
      background: lighten($theme-color-4, 50%);
    }
